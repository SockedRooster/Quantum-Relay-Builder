...
class QR_PT_builder(Panel):
    ...
    def draw(self, context):
        ...
        deploy = layout.box()
        deploy.label(text="Deployable Mount", icon='CONSTRAINT_BONE')
        deploy.prop(props, "generate_deployable_mount")
        deploy_controls = deploy.column()
        deploy_controls.enabled = props.generate_structure and props.generate_deployable_mount
        deploy_controls.prop(props, "deploy_arm_length")
        deploy_controls.prop(props, "deploy_arm_width")
        deploy_controls.prop(props, "deploy_arm_height")
        deploy_controls.prop(props, "deploy_hinge_radius")
        deploy_controls.prop(props, "deploy_base_radius")
        deploy_controls.prop(props, "deploy_base_height")
        deploy_controls.separator()
        deploy_controls.prop(props, "generate_deploy_braces")
        brace_controls = deploy_controls.column()
        brace_controls.enabled = props.generate_deploy_braces
        brace_controls.prop(props, "deploy_brace_width")
        brace_controls.prop(props, "deploy_brace_spread")
        deploy_controls.prop(props, "generate_deploy_actuator")
        actuator_controls = deploy_controls.column()
        actuator_controls.enabled = props.generate_deploy_actuator
        actuator_controls.prop(props, "deploy_actuator_radius")
        deploy_controls.separator()
        deploy_controls.prop(props, "deploy_angle")
        deploy_controls.separator()
        deploy_controls.label(text="Stage 1: Player-triggered deployment")
        deploy_controls.prop(props, "deploy_start_frame")
        deploy_controls.prop(props, "deploy_end_frame")
        deploy_controls.label(text="Stage 2: Indicator powers on")
        deploy_controls.prop(props, "power_on_frame")
